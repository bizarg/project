...
{
    const PRODUCT_NAME = "Replenish account";
    ...
    {
        parent::__construct(config('wayforpay.account'), config('wayforpay.secretKey'));
    }
    /**
     * Получаем: версию апи.
     *
    ...
    {
        $this->transactionType = $merchantTransactionType;
        return $this;
    }
    /**
    ...
    public function sendInvoice($order, $transactionType = null)
    {
        $request = $this->createForm($order, $transactionType)->generateInvoice();

        return  $this->validate($this->sendRequest($request));
    }

    public function generateInvoice()
    {
        $this->generateMerchantSignature();
    ...
    public function createForm($order, $transactionType = null)
    {
        $this->addProduct(self::PRODUCT_NAME, $order->amount, self::PRODUCT_COUNT)
            ->setMerchantDomainName(config('app.url'))
            ->setOrderReference($order->orderReference)
            ->setOrderDate($order->orderDate)
            ->setAmount($order->amount)
            ->setServiceUrl(config('app.url').config('wayforpay.service.invoice'))
            ->setClientEmail(User::find($order->user_id)->email);
        if($transactionType){
            $this->setApiVersion(1)
                ->setMerchantTransactionType('CREATE_INVOICE');
        } else {
            $this->setReturnUrl(config('app.url') . '/result');
        }
        return $this;
    }
    ...
    {
        $ch = curl_init(self::ADDRES_URL_WAY_WOR_PAY_INVOICE);
    ...
    public function validateResult($request)
    {
        if ($request->reason == 'Ok'
            && $request->transactionStatus == 'Approved'
            && $request->reasonCode == 1100) return true;

        return false;
    }

    public function returnAnswer($order)
    {
        $date = time();

        $signature = hash_hmac(
            'md5',
            $order->orderReference.';accept;'.$date,
